...
public class RentalServiceImplTest {
  ...
  public void findByName() {
    List<Customer> customers1 = rentalService.findByName("jones");
    List<Customer> customers2 = rentalService.findByName("johnson");
    List<Customer> customers3 = rentalService.findByName("abcdefgh");
    List<Customer> customers4 = rentalService.findByName("");
    assertTrue(customers1.stream().map(Customer::getId).collect(Collectors.toList()).containsAll(Arrays.asList(-1L, -2L, -4L)));
    assertFalse(customers1.stream().map(Customer::getId).collect(Collectors.toList()).contains(-3L));
    assertTrue(customers2.stream().map(Customer::getId).collect(Collectors.toList()).contains(-3L));
    assertTrue(customers2.stream().map(Customer::getId).noneMatch(id -> id == -1L || id == -2L || id == -4L));
    assertTrue(customers3.isEmpty());
    assertTrue(customers4.stream().map(Customer::getId).collect(Collectors.toList()).containsAll(Arrays.asList(-1L, -2L, -3L, -4L)));
  }

  @Test
  public void findByActor() {
    List<Film> films1 = rentalService.findByActorName("Carmen");
    List<Film> films2 = rentalService.findByActorName("laura");
    List<Film> films3 = rentalService.findByActorName("abcdefgh");
    List<Film> films4 = rentalService.findByActorName("");

    assertTrue(films1.stream().map(Film::getId).collect(Collectors.toList()).contains(-1L));
    assertTrue(films1.stream().map(Film::getId).noneMatch(id -> id == -2 || id == -3));
    assertTrue(films2.stream().map(Film::getId).collect(Collectors.toList()).containsAll(Arrays.asList(-1L, -2L)));
    assertTrue(films2.stream().map(Film::getId).noneMatch(id -> id == -3L));
    assertTrue(films3.isEmpty());
    assertTrue(films4.stream().map(Film::getId).collect(Collectors.toList()).containsAll(Arrays.asList(-1L, -2L)));
  }
}
